...
public void OnPluginStart()
{
	...
	g_hMeleeWeaponArray = new ArrayList(sizeof(MeleeWeaponInfo));
...
void StoreWeapons(int ent)
{
	...
	MeleeWeaponInfo esMeleeWeaponInfo;
	...
	else if (IdentifyMeleeWeapon(ent) != WEPID_MELEE_NONE)
	{
		GetMeleeWeaponName(IdentifyMeleeWeapon(ent), esMeleeWeaponInfo.name, sizeof(esMeleeWeaponInfo.name));
		GetEntPropVector(ent, Prop_Send, "m_vecOrigin", esMeleeWeaponInfo.origin);
		GetEntPropVector(ent, Prop_Send, "m_angRotation", esMeleeWeaponInfo.angle);
		g_hMeleeWeaponArray.PushArray(esMeleeWeaponInfo, sizeof(MeleeWeaponInfo));
	}
}
...
void ReplaceWeapons()
{
	...
	int iMeleeWeaponSize = g_hMeleeWeaponArray.Length;
	...
	for (int i = 0; i < iMeleeWeaponSize; i++)
	{
		g_hMeleeWeaponArray.GetArray(i, esMeleeWeaponInfo, sizeof(MeleeWeaponInfo));
		SpawnMeleeWeapon(esMeleeWeaponInfo.name, esMeleeWeaponInfo.origin, esMeleeWeaponInfo.angle);
	}
}
...
// from MeleeInTheSafeRoom.sp
void SpawnMeleeWeapon(char[] ScriptName, float origin[3], float angles[3])
{
	int iEntity = CreateEntityByName("weapon_melee_spawn");
	if (IsValidEntity(iEntity))
	{
		DispatchKeyValue(iEntity, "melee_script_name", ScriptName);
		TeleportEntity(iEntity, origin, angles, NULL_VECTOR);
		DispatchSpawn(iEntity);
		SetEntityMoveType(iEntity, MOVETYPE_NONE);
	}
