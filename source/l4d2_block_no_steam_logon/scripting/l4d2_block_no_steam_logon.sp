...
Handle
    g_hSDKCall_GetClientName = null,
    g_hSDKCall_IsTimingOut = null,
    g_hSDKCall_Disconnect = null,
    g_hSDKCall_GetNetChannel = null;
...
void InitGameData()
{
    ...
    g_iOS = gd.OS;
    if (g_iOS == OS_Unknown_Copy)
        SetFailState("Invalid operating system.");

    g_iOff_CBaseClient_m_Name = gd.GetOffset(OFFSET_NAME);

    delete (gd.CreateDetourOrFail(DETOUR_FUNCTION, true, DTR_OnValidateAuthTicketResponseHelper_Pre));
