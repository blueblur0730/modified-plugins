...
{
	name		= "L4D2 Item hint",
	author		= "BHaType, fdxx, HarryPotter, blueblur",
	description = "When using 'Look' in vocalize menu, print corresponding item to chat area and make item glow or create spot marker/infeced maker like back 4 blood. Support color difference for every individuals.",
	version		= "3.0-2024/3/6-edited-1.3",
 ...
 *
 * edited-1.0
 *  - Added cvar "l4d2_item_hint_use_config" to use config or not to specify player's color.
 *  - Added command "sm_setcolor <spot/item> <r> <g> <b>" to select color.
 *  - Added file "l4d2_item_hint.cfg" to store players' data.
 *
 * edited-1.1
 *  - Fixed some issue when config is not found.
 * 
 * edited-1.2
 *  - Added command "sm_resetcolor" to delete player's color data in the config.
 *  - Replaced multicolor.inc with color.inc.
 * 	- Defualt spot color is no longer black and invisible if we turn on the config cvar and there is no data by defualt in the config.
 * 	- Now if <spot/item> wasn't typed in correctly, "name" section will no longer write in the config.
 *  - Added description if the rgb number is out of range 0-255.
 * 
 * edited-1.3
 *  - Improved deletion method and empty value detection.
 */
...
#define CONFIG_PATH			  "configs/l4d2_item_hint.cfg"
...
bool		g_bItemInstructorHint, g_bSpotMarkInstructorHint, g_bInfectedMarkWitch, g_bUseConfig;
...
public void OnPluginStart()
{
	...
	g_hUseConfig				= CreateConVar("l4d2_item_hint_use_config", "1", "Use config to specify players by their SteamID to specify the color they ues? 1=on, 0=off", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	...
	g_hUseConfig.AddChangeHook(ConVarChanged_Cvars);

	RegConsoleCmd("sm_mark", CMD_MARK, "Mark item/infected/spot");
	RegConsoleCmd("sm_setcolor", CMD_COLOR, "Choose your own color (\"l4d2_item_hint_use_config\" has to be on).");
	RegConsoleCmd("sm_resetcolor", CMD_RESETCOLOR, "Reset your color to default.");
...
void GetCvars()
{
	...
	g_bUseConfig			 = g_hUseConfig.BoolValue;
}
...
{
	char sPosition[8], r[4], g[4], b[4];
	char sNumber[16];
	int test_r, test_g, test_b;

	if (!g_bUseConfig)
	{
		ReplyToCommand(client, "Server dose not allow players to select their own color of hint and marker.");
		return Plugin_Handled;
	}

	if (GetCmdArgs() != 4)
	{
		ReplyToCommand(client, "Usage: sm_setcolor <spot/item> <r> <g> <b>");
		return Plugin_Handled;
	}

	test_r = GetCmdArgInt(2);
	test_g = GetCmdArgInt(3);
	test_b = GetCmdArgInt(4);

	if (test_r > 255 || test_g > 255 || test_b > 255 || test_r < 0 || test_g < 0 || test_b < 0)
	{
		ReplyToCommand(client, "Usage: sm_setcolor <spot/item> <r> <g> <b>, rgb number has to be in range 0-255");
		return Plugin_Handled;
	}

	GetCmdArg(1, sPosition, sizeof(sPosition));
	GetCmdArg(2, r, sizeof(r));
	GetCmdArg(3, g, sizeof(g));
	GetCmdArg(4, b, sizeof(b));

	Format(sNumber, sizeof(sNumber), "%s %s %s", r, g, b);

	if (IsClientInGame(client) && !IsFakeClient(client))
	{
		KeyValues	Kv = new KeyValues("");
		static char sAuth[MAX_AUTHID_LENGTH];
		char		sName[MAX_NAME_LENGTH];

		if (!GetClientAuthId(client, AuthId_Steam2, sAuth, sizeof(sAuth)))
		{
			ReplyToCommand(client, "Your steamid is unauthorized. Process suspended.");
			return Plugin_Handled;
		}

		if (!GetClientName(client, sName, sizeof(sName)))
		{
			ReplyToCommand(client, "Failed to retrieve your name. Process suspended.");
			return Plugin_Handled;
		}

		if (!Kv.ImportFromFile(g_sPath))
		{
			ReplyToCommand(client, "Failed to import server file. Please contact admin to solve this issue");
			return Plugin_Handled;
		}

		if (Kv.JumpToKey(sAuth, true))
		{
			if (strcmp(sPosition, "spot") == 0) Kv.SetString("spot", sNumber);
			else if (strcmp(sPosition, "item") == 0) Kv.SetString("item", sNumber);
			else
			{
				ReplyToCommand(client, "Usage: sm_setcolor <spot/item> <r> <g> <b>, first argument must be \"spot\" or \"item\"");
				delete Kv;
				return Plugin_Handled;
			}
		...
		Kv.Rewind();
		if (Kv.ExportToFile(g_sPath))
			ReplyToCommand(client, "Colors has been set to your own!");
		else
			ReplyToCommand(client, "Unable to export file. Please contact admin to solve this");
...
public Action CMD_RESETCOLOR(int client, int args)
{
	if (IsClientInGame(client) && !IsFakeClient(client))
	{
		KeyValues	Kv = new KeyValues("");
		...
		if (Kv.JumpToKey(sAuth, false))
		{
			char sBuffer[12];
			do
			{
				if (Kv.GetSectionName(sBuffer, sizeof(sBuffer))) 
					Kv.DeleteKey(sBuffer);
			}
			while (Kv.GotoNextKey(false));
		}
		else
		{
			ReplyToCommand(client, "You haven't set a color for once before!");
			delete Kv;
			return Plugin_Handled;
		}
		...
		Kv.Rewind();
		if (Kv.ExportToFile(g_sPath))
			ReplyToCommand(client, "Your colors has been reset!");
		else
			ReplyToCommand(client, "Unable to export file. Please contact admin to solve this");
...
void CreateEntityModelGlow(int iEntity, const char[] sEntModelName, int client)
{
	int	 iClientColor;
	bool bUseDefualt = false;

	if (g_bUseConfig)
	{
		static char sAuth[MAX_AUTHID_LENGTH];
		char		sClientColor[16];
		KeyValues	Kv = new KeyValues("");
		...
		if (Kv.ImportFromFile(g_sPath))
		{
			if (Kv.JumpToKey(sAuth, false))
			{
				Kv.GetString("item", sClientColor, sizeof(sClientColor), "Failed");
				if (StrEqual(sClientColor, "Failed")) bUseDefualt = true;
				else iClientColor = GetColor(sClientColor);
			}
			else bUseDefualt = true;
		}
		else
			LogMessage("[l4d2_item_hint] Failed to import file %s", g_sPath);

		delete Kv;
	}
	else if (g_iItemCvarColor == 0) return;	   // no glow
	...
	if (g_bUseConfig && iClientColor != 0 && !bUseDefualt)
		SetEntProp(entity, Prop_Send, "m_glowColorOverride", iClientColor);
	else
...
void CreateSpotMarker(int client, bool bIsAimInfeced)
{
	...
	if (g_bUseConfig)
	{
		...
		if (Kv.ImportFromFile(g_sPath))
		{
			if (Kv.JumpToKey(sAuth, false))
			{
				Kv.GetString("spot", sClientColor, sizeof(sClientColor), "Failed");

				if (StrEqual(sClientColor, "Failed")) bUseDefualt = true;
				else
				{
					TrimString(sClientColor);
					iClientColorArray = ConvertRGBToIntArray(sClientColor);
				}
			}
			else bUseDefualt = true;
		}
	...
	else if (strlen(g_sSpotMarkCvarColor) == 0) return;	   // disable spot mark glow
	...
	int color[4];
	if (g_bUseConfig && !bUseDefualt)
	{
		color[0] = iClientColorArray[0];
		color[1] = iClientColorArray[1];
		color[2] = iClientColorArray[2];
		color[3] = 255;
	}
	else
	{
	...
	if (CheckIfEntitySafe(infoTarget))
	{
		...
		if (CheckIfEntitySafe(sprite))
		{
			...
			if (g_bUseConfig && !bUseDefualt)
				DispatchKeyValue(sprite, "rendercolor", sClientColor);
			else
			...
			if (GetEntPropString(iEntity, Prop_Data, "m_ModelName", sEntModelName, sizeof(sEntModelName)) > 1)
			{
				...
				if (bIsVaildItem)
				{
					if (GetEngineTime() > g_fItemHintCoolDownTime[client])
					{
						...
						CreateEntityModelGlow(iEntity, sEntModelName, client);
